...
tree_equal (tree *t1, tree *t2)
{
  ...
  if (t1->data != t2->data)
    return false;
  return (tree_equal (t1->left, t2->left)
...
void
tree_set_left (tree *t, tree *left)
{
  assert (t);
  assert (t->left == NULL);
...
void
tree_set_right (tree *t, tree *right)
{
  assert (t);
  assert (t->right == NULL);
...
void
tree_set_value (tree *t, void *data)
{
  assert (t);
  int val = 0;
  t->data = data;
...
void
tree_free_full (tree *t, void (*destroy) (void *))
{
  if (t && t->left)
    tree_free_full (t->left, destroy);
  if (t && t->right)
    tree_free_full (t->right, destroy);
  if (t && t->parent)
    t->parent = NULL;
  destroy (t->data);
  free (t);
